	 ...
	 @PostMapping("/add/question")
	    public ResponseEntity<String> addNewQuestion(@RequestBody Question question) {
	        try {
	            ...
	            Optional<Subject> optionalSubject = subjectRepository.findById(question.getSname_subject_name().getName());
	            if (optionalSubject.isPresent()) {
	                ...
	                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Invalid subject ID: " + question.getSname_subject_name().getName());
	            }
	        } catch (Exception e) {
